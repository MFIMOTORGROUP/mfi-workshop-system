...
export default function VehiclesPage() {
  ...
  const exportToCSV = () => {
    const headers = [
      "Make","Model","Reg","Mileage","Purchase Price","CAP Clean","CAP Live",
      "Status","MOT","Transmission","Grade","V5C","Keys","Sold Date",
    ];

    const rows = filteredVehicles.map((v) => [
      v.make,v.model,v.reg,v.mileage,v.purchase_price,
      v.cap_clean_price,v.cap_live_price,v.status,v.mot,
      v.transmission,v.grade,v.v5c_status,v.keys_count,v.sold_date,
    ]);
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-600">
              <tr>
                {["Make","Model","Reg","Mileage","Purchase","CAP Clean","CAP Live","Status","MOT","Transmission","Grade","V5C","Keys","Sold Date","Days","Margin %","CAP Check","Edit","Toggle"].map((h) => (
              ...
              {filteredVehicles.map((v) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">{v.mileage}</td>
                    <td className="px-6 py-4 text-right">£{v.purchase_price}</td>
                    <td className="px-6 py-4 text-right">£{v.cap_clean_price}</td>
                    <td className="px-6 py-4 text-right">£{v.cap_live_price}</td>
                    <td className="px-6 py-4">{v.status}</td>
                    <td className="px-6 py-4">{v.mot ? formatDate(v.mot) : "-"}</td>
                    ...
                    <td className="px-6 py-4">{days} days</td>
                    <td className="px-6 py-4">{margin}%</td>
                    <td className="px-6 py-4">{capWarning}</td>
                    <td className="px-6 py-4">
                      <button
                        onClick={() => {
                          setEditingVehicle(v);
